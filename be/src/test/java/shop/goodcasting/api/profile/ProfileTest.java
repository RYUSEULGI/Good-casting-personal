...
//
    ...
    @Test
    public void test() {
        Double cmdel = Double.valueOf("17.1"); //키 "cm" 삭제
        int test = (int) Math.round(cmdel);
    ...
    public void createprofile() throws Exception {
        ...
        for (int i = 1; i < list.size(); i++) {
            ...
            String yeardel = birthday.text().replace("년", "").trim(); //출생년도 "년" 삭제
            boolean human = (height.text().toUpperCase().contains("CM") && weight.text().toUpperCase().contains("KG"));
            if (human) {
                Double cmdel = Double.valueOf((height.text().toUpperCase().replace("CM", "")).trim()); //키 "cm" 삭제
                int finalcm = (int) Math.round(cmdel);

                Double kgdel = Double.valueOf((weight.text().toUpperCase().replace("KG", "")).trim());
                int finalkg = (int) Math.round(kgdel);
                int finalAge = 2021 - Integer.parseInt(yeardel);
                ...
                actorDTO.setHeight(finalcm);
                actorDTO.setWeight(finalkg);
                actorDTO.setName(name.text());
                actorDTO.setAgency(i + "컴퍼니");
                actorDTO.setPhone("010-" + i);
                actorDTO.setAge(finalAge);
                actorDTO.setEmail("actor"+i+"@navar.com");
                ...
                FileOutputStream fos = new FileOutputStream("/usr/local/goodCasting/" +name.text()+".jpg");
                ...
                String saveName = uploadPath + "/" + name.text()+".jpg";
                Path savePath = Paths.get(saveName);
                System.out.println("image thumbnail extract");
                String thumbnailSaveName = uploadPath + "/" + "s_" + uuid + "_" + name.text() + ".jpg";
